...
@role_bp.route('/add', methods=['POST', 'GET'])
def add():	
	if g.user:
...
@role_bp.route('/<public_id>/update', methods=['POST', 'GET'])
def update(public_id):
	if g.user:
		url1 = api_url+'/role/'+public_id
		headers = { 
			'Authorization' : '{}'.format(session['token']) 
		}
		...
		if request.method == 'POST':
			...
			if session['role'] == 'Main Admin' or json_data1['username'] == session['user']:
				...
				url = api_url+'/role/'+public_id+'/update'
				headers = { 
					'Authorization' : '{}'.format(session['token']) 
				}
				payload = {
...
@role_bp.route('/<public_id>/delete')
def delete(public_id):
	if g.user:
		headers = { 'Authorization' : '{}'.format(session['token']) }
		url = api_url+'/role/'+public_id+'/delete'
		files = {
